...
controller_interface::InterfaceConfiguration Hk1dMockRobot::
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  // Get time and periode as double
  // double current_time = ...;
  double dt = period.seconds();

  // Simulate mock system
  command_mock_external_force_ = 0.0;
  command_mock_joint_velocity_ = current_joint_velocity_ + last_commanded_joint_force_*dt;
  command_mock_joint_position_ = current_joint_position_ + command_mock_joint_velocity_*dt;
...
bool Hk1dMockRobot::read_state_from_hardware()
{
  ...
  if (nan_last_commanded_joint_force) {
    ...
    RCLCPP_WARN_THROTTLE(get_node()->get_logger(), *clock, 1000, "State commanded joint force is NaN!");
    // return false;
  }
  return true;
}
