...
#include <Python.h>
#include "hk1d_mock_pyrobot/hk1d_mock_pyrobot_logic.h"

namespace hk1d_mock_pyrobot
{
Hk1DMockPyrobot::Hk1DMockPyrobot()
...
controller_interface::CallbackReturn Hk1DMockPyrobot::on_init()
{
  try
  {
    declare_parameters();
  ...
  if (PyImport_AppendInittab("hk1d_mock_pyrobot_logic", PyInit_hk1d_mock_pyrobot_logic) == -1) {
    fprintf(stderr, "Error: could not extend in-built modules table\n");
    return controller_interface::CallbackReturn::ERROR;
  }

  Py_Initialize();
  PyImport_ImportModule("hk1d_mock_pyrobot_logic");

  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn Hk1DMockPyrobot::on_configure(
...
Hk1DMockPyrobot::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration Hk1DMockPyrobot::state_interface_configuration()
  const
{
...
controller_interface::CallbackReturn Hk1DMockPyrobot::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
controller_interface::CallbackReturn Hk1DMockPyrobot::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
controller_interface::return_type Hk1DMockPyrobot::update(
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  // run cython function
  if (hk1d_mock_pyrobot_logic(period.seconds(), states_, commands_, message, params_)) {
    RCLCPP_ERROR_THROTTLE(
      get_node()->get_logger(), *(get_node()->get_clock()), 1000,
      "hk1d_mock_pyrobot logic failed.");
...
void Hk1DMockPyrobot::declare_parameters()
{
  param_listener_ = std::make_shared<ParamListener>(get_node());
}

controller_interface::CallbackReturn Hk1DMockPyrobot::read_parameters()
{
  if (!param_listener_)
  {
...
PLUGINLIB_EXPORT_CLASS(
  hk1d_mock_pyrobot::Hk1DMockPyrobot, controller_interface::ControllerInterface)
